...
DADADADUM: Tuple[str, ...]
"""Melodia: l'obertura de la cinquena simfonia de Beethoven en Do menor (C minor)."""
...
BADDY: Tuple[str, ...]
"""Melodia: entrada d'un dolent a l'era del cinema mut. (dolent (baddy))"""
CHASE: Tuple[str, ...]
"""Melodia: escena de persecució de l'era del cinema mut. (persecució (chase))"""
BA_DING: Tuple[str, ...]
"""Melodia: senyal breu per indicar que alguna cosa ha passat."""
WAWAWAWAA: Tuple[str, ...]
"""Melodia: un trombó molt trist."""
JUMP_UP: Tuple[str, ...]
"""Melodia: per utilitzar-se en un joc, indicant moviment cap amunt. (jump up (saltar cap amunt))"""
JUMP_DOWN: Tuple[str, ...]
"""Melodia: per utilitzar en un joc, que indica moviment cap avall. (jump down (saltar cap avall))"""
POWER_UP: Tuple[str, ...]
"""Melodia: una fanfàrria per indicar un assoliment desbloquejat. (engegar (power up))"""
POWER_DOWN: Tuple[str, ...]
"""Melodia: una trista fanfàrria per indicar un assoliment perdut. (apagar (power down))"""

def set_tempo(ticks: int=4, bpm: int=120) -> None:
    """Estableix el tempo aproximat per la reproducció. (set tempo (estableix el tempo))

Example: ``music.set_tempo(bpm=120)``

:param ticks: (tics (ticks)) El nombre de tics que constitueixen un ritme.
:param bpm: (ppm pulsacions per minut (bpm en anglès)) Un nombre enter determinant quantes pulsacions per minut.
...
:param loop: (bucle) Si el ``loop`` (bucle)s'estableix en ``True`` (Cert), la melodia es repeteix fins que es crida ``stop`` (atura) o s'interromp la trucada de bloqueig.
...
:param pin: (pin (punt de connexió)) Pin de sortida opcional (``pin0`` per defecte).
:param wait: (espera) Si ``wait`` s'estableix en ``True``, aquesta funció està bloquejant.
...
:param pin: (punt de connexió (pin)) Es pot proporcionar un argument opcional per especificar un pin, per exemple ``music.stop(pin1)``."""
