...
def pack_into(fmt: str, buffer: WriteableBuffer, offset: int, v1: Any, *vn: Any) -> None:
    """포맷 문자열에 따라 값을 패킹합니다. (pack into)

Example: ``struct.pack_info('hf', buffer, 1, 3.1415)``

:param fmt: (fmt) 포맷 문자열.
:param buffer: (buffer) 작성할 목표 버퍼.
:param offset: (offset) 버퍼에 적용할 오프셋. 음수를 입력해 버퍼 끝부터 셀 수 있습니다.
:param v1: (v1) 첫 값.
:param *vn: (*vn) 남은 값."""
    ...

def unpack(fmt: str, data: ReadableBuffer) -> Tuple[Any, ...]:
    """포맷 문자열에 따라 데이터를 언패킹합니다. (unpack)
...
def unpack_from(fmt: str, buffer: ReadableBuffer, offset: int=0) -> Tuple:
    """포맷 문자열에 따른 버퍼로부터 데이터를 언패킹합니다. (unpack from)

Example: ``v1, v2 = struct.unpack_from('hf', buffer)``

:param fmt: (fmt) 포맷 문자열.
:param buffer: (buffer) 읽을 소스 버퍼.
:param offset: (오프셋) 버퍼에 적용할 오프셋. 음수를 입력해 버퍼 끝부터 셀 수 있습니다.
:return: A tuple of the unpacked values."""
    ...