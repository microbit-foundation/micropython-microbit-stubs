...
:param length: (length) (기본=32) 라디오를 통해 전송되는 메시지의 최대 길이를 바이트로 정의합니다.
최대 251바이트까지 정의할 수 있습니다(S0, LENGTH 및 S1 preamble의 경우 254 - 3바이트).
:param queue: (queue) (기본=3) 수신 메시지 대기열에 보관할 수 있는 메시지의 수를 특정합니다. 만약 수신 메시지의 대기열 공간이 부족하다면 수신 메시지는 드롭됩니다.
...
def send_bytes(message: bytes) -> None:
    """바이트가 포함된 메시지를 보냅니다. (send bytes)

Example: ``radio.send_bytes(b'hello')``

:param message: (message) 보낼 바이트."""
    ...
...
:param buffer: (buffer) 목표 버퍼. 메시지가 버퍼보다 크면 메시지가 잘립니다.
...
:param message: (message) 보낼 메시지."""
    ...

def receive() -> Optional[str]:
    """``receive_bytes``와 정확히 동일한 작업을 하지만 전송된 모든 것을 반환합니다. (receive)
