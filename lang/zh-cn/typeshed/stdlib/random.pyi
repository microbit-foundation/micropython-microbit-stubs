...
def randint(a: int, b: int) -> int:
    """在``a``到``b``（包含``b``）之间随机选择一个整数。 (randint)
...
def randrange(stop: int) -> int:
    """在零到``stop``（不含``stop``）之间随机选择一个整数。 (randrange)

Example: ``random.randrange(10)``

:param stop: (stop) 区间结束值（不含）"""
    ...

@overload
def randrange(start: int, stop: int, step: int=1) -> int:
    """从区间``range(start, stop, step)``中随机选择一个元素。 (randrange)

Example: ``random.randrange(0, 10)``

:param start: (start) 区间起始值（包含）
:param stop: (stop) 区间结束值（不含）
:param step: (step) 步长。"""
    ...
_T = TypeVar('_T')
...
def uniform(a: float, b: float) -> float:
    """返回一个介于``a``和``b``（包含``b``）之间的随机浮点数。 (uniform)
