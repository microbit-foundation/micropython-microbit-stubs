"""使用内置无线电在 micro:bits 之间进行通信。 (radio)"""
from _typeshed import WriteableBuffer
from typing import Optional, Tuple
RATE_1MBIT: int
"""用于表示每秒 1 兆字节吞吐量的常数。 (rate 1mbit)"""
RATE_2MBIT: int
"""用于表示每秒 2 兆字节吞吐量的常数。 (rate 2mbit)"""
...
:param length: (length) （默认值32）以字节为单位，定义了通过无线电传输消息的最大长度。
其长度可达 251 字节（S0、LENGTH 和 S1 前导码：254-3 字节）。
:param queue: (queue) （默认值3）规定了传入消息队列中可以存储的消息数量。
如果队列中没有多余的空间留给传入的消息，那么传入的消息就会被丢弃。
:param channel: (channel) （缺省值7）范围为从0到83（含83）的整数，用于定义一任意“信道”，以便将无线电调到该信道。信息通过该信道发送，并且只有通过该信道接收到的消息才被放到传入消息队列中。每一步的宽度为 1 MHz，基于 2400 MHz。
:param power: (power) （默认值6）用于表示广播消息时所使用信号强度的整数值，其范围为从0到7（包含7）。
...
:param group: (group) （默认值0）过滤消息时与 ``address`` 一起使用的 8 位的数值（0-255）。
从概念上讲，"address”就像房屋或办公室的地址，而“group”就像住在该地址，并且你想给他或她发消息的那个人。
:param data_rate: (data rate) （默认值``radio.RATE_1MBIT``）表示数据吞吐速度。
...
def send_bytes(message: bytes) -> None:
    """发送一条包含字节的消息。 (send bytes)

Example: ``radio.send_bytes(b'hello')``

:param message: (message) 待发送的字节。"""
    ...

def receive_bytes() -> Optional[bytes]:
    """接收消息队列中的下一条传入消息。 (receive bytes)
...
def receive_bytes_into(buffer: WriteableBuffer) -> Optional[int]:
    """将消息队列中的下一条传入消息复制到缓冲区。 (receive bytes into)

Example: ``radio.receive_bytes_info(buffer)``

:param buffer: (buffer) 目标缓冲区。如果消息大小大于缓冲区，则消息会被截断。
...
:param message: (message) 待发送的字符串。"""
    ...

def receive() -> Optional[str]:
    """按照与``receive_bytes``完全相同的工作方式，但是返回所发送的内容。 (receive)
...
def receive_full() -> Optional[Tuple[bytes, int, int]]:
    """返回一个包含三个数值的元组，用来代表消息队列中的下一条传入消息。 (receive full)
