...
:param length: (length) (デフォルト=32)\u3000無線を介して送信されるメッセージのバイト単位の最大長を設定します。最大で 251 バイト(254 から S0、長さ、S1 プリアンブルを引いた値)にできます。
:param queue: (queue) (デフォルト=3)\u3000受信メッセージキューに格納できるメッセージの数を指定します。着信メッセージのキューに空きがない場合、着信メッセージは捨てられます。
...
:param power: (power) (デフォルト=6)\u3000メッセージをブロードキャストするときに使用される信号の強度を示すもので、0 から 7 までの整数値を設定できです。
値が高いほど信号は強くなりますが、デバイスが消費する電力が大きくなります。指定の番号は次のリストの dBm (デシベルミリワット)値の位置に変換されます: -30, -20, -16, -12, -8, -4, 0, 4 。
:param address: (address) (デフォルト=0x75626974)\u300032 ビットのアドレスとして表される任意の名前であり、ハードウェアレベルで着信パケットをフィルタリングするために使用されます。フィルタリングはユーザーが設定したアドレスと一致するもののみを維持します。他のmicro:bit関連のプラットフォームで使われるデフォルトは、ここで使われるデフォルト設定となっています。
:param group: (group) (デフォルト=0)\u30008ビットの値(0〜255)であり、 ``address`` フィルタしたメッセージで使います。概念的に "address" は自宅/事務所の住所のようなものであり、 "group" はその住所のメッセージを受け取人のようなものです。
:param data_rate: (data rate) (デフォルト=``radio.RATE_1MBIT``)\u3000データスループットが起こる速度を示しています。``radio`` モジュールに定義されている定数 ``RATE_250KBIT``、``RATE_1MBIT``、``RATE_2MBIT`` のいずれかを指定します。
...
def receive_bytes_into(buffer: WriteableBuffer) -> Optional[int]:
    """メッセージキューにある次の着信メッセージを受信します。メッセージは buffer にコピーされます。 (receive bytes into)
